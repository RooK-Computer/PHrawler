...
function KeyboardInput:registerPlayer(player, controls)
	...
	for command, key in pairs(keyboardInputs) do
		...
		if
			command == Constants.PLAYER_DIRECTION_LEFT
			or command == Constants.PLAYER_DIRECTION_RIGHT
			or command == Constants.PLAYER_FIGHT_COMMAND
		then
...
function KeyboardInput:keypressed(pressedKey, scancode, isrepeat)
	...
	if pressedKey == "." then
		game.activateDebug = not game.activateDebug
	end
